...
// --- Global Variables ---
const PARALLAX_SPEED = 0.5; // Background moves at half speed
let backgrounds = []; // Array to hold background sprites
let backgroundScale = 1; // Store scale for use in update
...
let backgroundMusic = null; // NEW: Holds the music instance
let hasStartedMusic = false; // NEW: Only start music once per session
let audioButton = null; // NEW: Audio toggle button
let isMusicMuted = false; // NEW: Track mute state
...
let doubleJumpCooldownActive = false; // <-- FIX: should be false, not true
...
window.addEventListener('load', function() {
    ...
    // Helper for iOS Safari fullscreen (standalone mode workaround)
    function isIos() {
    ...
    // Only show fullscreen button on mobile devices that support fullscreen or iOS Safari
    if (isMobile && (canFullscreen() || isIos())) {
        fullscreenBtn.style.display = 'block';

        fullscreenBtn.addEventListener('click', async function() {
            // iOS Safari: suggest "Add to Home Screen" if not standalone
            if (isIos() && isSafari() && !isInStandaloneMode()) {
                alert("For the best fullscreen experience on iPhone/iPad, tap the Share button and choose 'Add to Home Screen'.");
                return;
            }

            // Try all fullscreen APIs for best compatibility
            ...
            try {
                ...
                // Fallback: show alert for iOS users
                if (isIos()) {
                    alert("Fullscreen is limited on iOS Safari. For best results, use 'Add to Home Screen'.");
                }
            }
        });
    }
...
function preload() {
  ...
  this.load.audio('bgm', 'assets/song.m4a'); // NEW: Preload background music
  this.load.image('background', 'assets/background.png'); // NEW: Load background image
}
...
function create() {
  ...
  this.events.once('fontsloaded', () => {
    ...
    // --- Parallax Background (auto-tile to fill width, no gaps) ---
    backgrounds = [];
    // Create a temporary image to get its width/height
    const tempBg = this.add.image(0, 0, 'background').setVisible(false);
    backgroundScale = config.height / tempBg.height;
    const bgDisplayWidth = tempBg.width * backgroundScale;
    const numBgTiles = Math.ceil(config.width / bgDisplayWidth) + 3;
    tempBg.destroy();

    for (let i = 0; i < numBgTiles; i++) {
      const bg = this.add.image(i * bgDisplayWidth, 0, 'background')
        .setOrigin(0, 0)
        .setDepth(-2)
    ...
    // --- Add dark blur overlay above background ---
    // This rectangle overlays the background and gives a dark, soft blur effect
    const bgOverlay = this.add.rectangle(
      0, 0, config.width, config.height,
      0x000000, 0.38 // 38% opacity, tweak as needed
    )
      .setOrigin(0, 0)
      .setDepth(-1)
      .setScrollFactor(0);

    // Optionally, add a blur effect if supported (Phaser 3 WebGL only)
    // This is a soft fallback for most browsers, but not all platforms support filters.
    if (this.sys.game.renderer instanceof Phaser.Renderer.WebGL.WebGLRenderer) {
      try {
        // Use Phaser's built-in BlurPostFXPipeline if available
        if (this.plugins.hasPlugin('rexblurpipelineplugin')) {
          // If using rexrainbow's blur pipeline plugin
          bgOverlay.setPipeline('rexBlurPostFx');
        } else if (Phaser.Renderer.WebGL.Pipelines.BlurPostFXPipeline) {
          // Phaser 3.60+ built-in blur
          bgOverlay.setPostPipeline(Phaser.Renderer.WebGL.Pipelines.BlurPostFXPipeline);
        }
      } catch (e) {
        // If blur is not available, just use the dark overlay
      }
    }
    ...
    jumpsAvailable = 2;
    doubleJumpCooldownActive = false; // <-- FIX: should be false, not true
    ...
    // Add "DOUBLE JUMP" text above the cooldown bar, with improved readability
    this.add.text(
      jumpButton.x,
      jumpButton.y - BUTTON_SIZE / 2 - 27, // Slightly more vertical space
      "DOUBLE JUMP",
      {
        fontSize: "18px",
        fill: "#fff",
    ...
    audioButton = this.add.container(
        AUDIO_BTN.MARGIN + AUDIO_BTN.SIZE / 2,
        AUDIO_BTN.MARGIN + AUDIO_BTN.SIZE / 2
    ).setDepth(1001).setScrollFactor(0);

    // Simple graphics for the icon
    const audioIcon = this.add.graphics();
    function drawAudioIcon(muted = false) {
        audioIcon.clear();
        audioIcon.lineStyle(AUDIO_BTN.LINE_WIDTH, AUDIO_BTN.COLOR);

        if (!muted) {
            // --- Draw three vertical "wave" lines of different heights ---
            const waveSpacing = 6;
            const heights = [10, 16, 7]; // left, middle, right
            const xs = [-waveSpacing, 0, waveSpacing];
            for (let i = 0; i < 3; i++) {
                audioIcon.beginPath();
                audioIcon.moveTo(xs[i], heights[i] / 2 * -1);
                audioIcon.lineTo(xs[i], heights[i] / 2);
                audioIcon.strokePath();
            }
        } else {
            ...
            const gap = 4;
            audioIcon.beginPath();
            audioIcon.moveTo(-gap - barWidth / 2, -barHeight / 2);
            audioIcon.lineTo(-gap - barWidth / 2, barHeight / 2);
            audioIcon.moveTo(-gap + barWidth / 2, -barHeight / 2);
            audioIcon.lineTo(-gap + barWidth / 2, barHeight / 2);
    ...
    // Initial draw
    drawAudioIcon(isMusicMuted);
    audioIcon.setPosition(0, 0).setScale(AUDIO_BTN.ICON_SCALE);
    audioButton.add(audioIcon);

    // Make interactive with simple hover effect
    const hitArea = new Phaser.Geom.Circle(0, 0, AUDIO_BTN.SIZE / 2);
    audioButton.setInteractive(hitArea, Phaser.Geom.Circle.Contains);

    audioButton
        .on('pointerover', () => {
            audioIcon.setAlpha(AUDIO_BTN.HOVER_ALPHA);
        })
        .on('pointerout', () => {
            audioIcon.setAlpha(AUDIO_BTN.ALPHA);
        })
        .on('pointerdown', () => {
    ...
    if (!backgroundMusic) {
      backgroundMusic = this.sound.add('bgm', { loop: true, volume: 0.45 });
    }
    // Try to unlock audio context on user gesture (required by many browsers)
    ...
    // Play or resume music unless muted
    if (!backgroundMusic.isPlaying && !isMusicMuted) {
      backgroundMusic.play();
    } else if (backgroundMusic.isPlaying && isMusicMuted) {
      backgroundMusic.pause();
    } else if (!backgroundMusic.isPlaying && isMusicMuted) {
      // If muted, ensure it's paused
      backgroundMusic.pause();
    }
...
function spawnInvincibilityPowerUp() {
  ...
  // --- Use power.png icon for power-up (no glow, no container) ---
...
function gameOver() {
  ...
  // --- Do NOT stop or pause background music on game over ---
...
function update(time, delta) {
  ...
  // --- Parallax Background Scrolling (auto-tile, no gaps) ---
  const scrollAmount = scrollSpeed * PARALLAX_SPEED * delta * 0.016;
  const bgDisplayWidth = backgrounds[0].width * backgroundScale;
  backgrounds.forEach(bg => {
    bg.x -= scrollAmount;
  });
  // Loop backgrounds if they move off screen
  // Always keep backgrounds sorted by x
  backgrounds.sort((a, b) => a.x - b.x);
  for (let i = 0; i < backgrounds.length; i++) {
    const bg = backgrounds[i];
    if (bg.x <= -bgDisplayWidth) {
      // Find the rightmost background
      let rightmost = backgrounds[backgrounds.length - 1];
      bg.x = rightmost.x + bgDisplayWidth;
      // Resort after moving
      backgrounds.sort((a, b) => a.x - b.x);
    }
  }
